...
import { LOGO_BASE64, LOGO_FORMATO } from './logo-data.js'
...
export async function gerarPdfContrato(texto) {
  ...
  // Embute a logo uma vez.
  let logoImg = null
  const logoBytes = carregarLogo()
  if (logoBytes) {
    try {
      logoImg = LOGO_FORMATO === 'jpg'
  ...
  // Cabeçalho com a logo (só na primeira página).
  function desenharCabecalho() {
    const topo = pageH - margin
    if (logoImg) {
      const logoW = 200
      const logoH = (logoImg.height / logoImg.width) * logoW
      const x = (pageW - logoW) / 2 // centralizada
      page.drawImage(logoImg, { x, y: topo - logoH, width: logoW, height: logoH })
      // Linha separadora
      page.drawLine({
        start: { x: margin, y: topo - logoH - 14 },
        end: { x: pageW - margin, y: topo - logoH - 14 },
        thickness: 0.6,
